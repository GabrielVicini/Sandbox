...
class wasm2lua extends stringcompiler_1.StringCompiler {
                ...
                case "Instr": {
                    switch (ins.id) {
                        ...
                        case "get_local": {
                            ...
                            // An earlier assignment only guarantees this local is initialized if it
                            // happened in a block that is still open at this point. Once that block
                            // has closed, a branch out of it can reach this get with the local never
                            // assigned. Wasm says such a local reads zero; an unassigned Lua local
                            // reads nil, which then gets stored into memory as a nil cell. In that
                            // case zero it at function entry, where wasm zeroes it -- NOT at the get,
                            // which would clobber the value on paths that did assign it.
                            let assignedInClosedBlock = (data != null) && (data[1] !== undefined)
                                && (state.blocks.indexOf(data[1]) === -1);
                            if ((data == null || assignedInClosedBlock) && (locID >= (state.funcType ? state.funcType.params.length : 0))) {
                                let forceInitIns = 1;
                                if (!assignedInClosedBlock) {
                                    forceInitIns = state.insCountPass1;
                                ...
                                let initList = state.forceVarInit.get(forceInitIns);
                                if (initList.indexOf(locID) === -1) {
                                    initList.push(locID);
                                }
                            }
                ...
                case "Instr": {
                    switch (ins.id) {
                        ...
                        case "store32": {
                            ...
                            let loadOffset = (ins.args[0] && ins.args[0].value)
    || (ins.namedArgs && ins.namedArgs.offset && ins.namedArgs.offset.value)
    || 0;
                            ...
                            if (targ) {
                                ...
                                if (ins.object == "u32") {
                                    ...
                                    if (tmp.startsWith("__LONG_INT__")) {
                                        ...
                                        // NOTE: must not stage this through a temp register. Both operands
                                        // have already been popped, so their registers are free and the temp
                                        // can alias the address register, clobbering it before it is read.
                                        this.write(buf, `(${tmp} or __LONG_INT__(0,0)):${ins.id}(${targ},${tmp2}${loadOffsetStr});`);
                                    }
                                }
    ...
    || 0;
                        ...
                        case "memory.copy": {
                            let targ = state.modState.memoryAllocations.get(0);
                            let len = this.getPop(state);
                            let src = this.getPop(state);
                            let dst = this.getPop(state);
                            this.write(buf, `__MEMORY_COPY__(${targ},${dst},${src},${len});`);
                            this.newLine(buf);
                            break;
                        }
                        case "memory.fill": {
                            let targ = state.modState.memoryAllocations.get(0);
                            let len = this.getPop(state);
                            let val = this.getPop(state);
                            let dst = this.getPop(state);
                            this.write(buf, `__MEMORY_FILL__(${targ},${dst},${val},${len});`);
                            this.newLine(buf);
                            break;
                        }
                        default: {
    ...
    processModuleExport(node, modState) {
        ...
        switch (node.descr.exportType) {
            ...
            default: {
                if (node.descr.exportType === "Memory") { return; }
